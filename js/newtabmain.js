gd.settingDataDef().done(() => {
  ...
  (function($, window, moduleName) {
    ...
    function renderMV() {
      ...
      for (let i = 0; i < mv.length; i++) {
        let obj = mv[i];
        html += `<div><a href='${obj.url}'>${obj.title}</a></div>`;
      }
    ...
    function showEditLayer(isAdd) {
      ...
      layx.html(
        "add-mv-modal",
        title,
        document.getElementById("add-mv-modal"),
        {
          width: 280,
          height: 70,
  ...
  (function($, window, moduleName) {
    ...
    function renderMV() {
      ...
      for (let i = 0; i < mv.length; i++) {
        let obj = mv[i];
        html += `<div id='{0}'>{1}</div>`.format(obj.id, obj.content);
      }
  ...
  (function($, window, moduleName) {
    ...
    function bindPagesEvent(isBlog) {
      $(".pager a", isBlog ? "#tabs-blogs" : "#tabs-news").on(
        "click",
        function() {
          let e = $(this);
          if (e.hasClass("current")) return false;
          let reg = /\/news\/page\/(?<page>\d+)\//gi;
          if (isBlog) reg = /\/blog\/page\/(?<page>\d+)\//gi;
    ...
    function getNewsByPageIndex(pageIndex) {
      $.get("https://home.cnblogs.com/news/page/" + pageIndex).then(
        function(data) {
          let reg = /<div class="news_item" id="news_item\d+">\s+<h2 class="news_entry">\s+<a href="\/\/(?<url>news.cnblogs.com\/n\/\d+\/)" target="_blank">(?<title>.*?)<\/a>/gim;
          let html = "<ul>";
    ...
    function getBlogsByPageIndex(pageIndex) {
      $.get("https://home.cnblogs.com/blog/page/" + pageIndex).then(
        function(data) {
  ...
  (function($, window, moduleName) {
    ...
    let dayCount = 0;
    $(function() {
      $.get("http://t.weather.sojson.com/api/weather/city/101280601").then(
        function(data) {
          let html = "";
          for (let i = 0; i < data.data.forecast.length; i++) {
            html += getDayWeatherHtml(
              data.data.forecast[i],
              data.data,
              i === 0
            );
          }
    ...
    function getDayWeatherHtml(data, ssData, isFirst) {
      //   let dayMatch = /(?<day>\d{1,2})日(?<weekday>星期[\u4e00-\u9fa5]+)/.exec(
      //     data.date
      //   );
      let day = data.date.toInt();
      let weekday = data.week;
      ...
      return `<div class="weatherMoudle {0}">
        ...
        isToday
          ? ""
          : `<div class="weather_week">{0}</div><div class="weather_date">{1}</div>`.format(
              weekday,
              new Date().addDay(day - nowDay).format("MM月dd日")
            ),
        data.type,
        isToday
          ? `<div class="weather_shishi">
          ...
          : "",
        `{0} - {1} ℃`.format(
          /\d{1,2}/.exec(data.low)[0],
          /\d{1,2}/.exec(data.high)[0]
        ),
        data.type,
  ...
  (function($, window) {
    ...
    function redrawStyle() {
      ...
      $("#dock").css(
        "left",
        (document.body.clientWidth - $("#dock").cssVal("width")) / 2
      );
    }
          ...
          buttons: [
            {
              ...
              callback: function(id, button, event) {
                ...
                moudleShowObj.each(function() {
                  let obj = gd.setting.module.findObjOfComplexObj(
                    "name",
                    $(this).attr("id")
                  );
                  ...
                  switch (obj.name) {
                    case "cnblogs":
                      ...
                      obj.data.defaultExpand = $(
                        "#cb-defaultExpand-cnblogs",
                        contentWindow.document.body
                      ).prop("checked");
                      break;
